...
/**
 * A convenient wrapper class for AProxyI that provides a fluent API
 ...
 * ```kotlin
 * AProxyIWrapper.track("https://api.example.com/users", "GET") {
 ...
 * ```kotlin
 * val tracker = AProxyIWrapper.request()
 ...
 * ```kotlin
 * AProxyIWrapper.trackSuspend("https://api.example.com/users", "GET") {
 *     apiService.getUsers()
 * }
 * ```
 */
object AProxyIWrapper {
    ...
    /**
     * Initialize the AProxyI. Call this in your Application.onCreate()
     */
    @JvmStatic
    fun init(context: Context, config: AProxyIConfig = AProxyIConfig.DEBUG) {
        AProxyI.init(context, config)
    }
     ...
     */
    @JvmStatic
    fun isEnabled(): Boolean = AProxyI.isEnabled()
    ...
    inline fun <T> track(
        ...
        val requestId = AProxyI.onRequestStart(
            url = url,
            method = method,
            headers = headers,
            body = body
        ...
        return try {
            val result = block()
            AProxyI.onRequestSuccess(requestId, 200, result)
            result
        } catch (e: Exception) {
    ...
    inline fun <T> trackWithCode(
        ...
        return try {
            ...
            if (code in 200..299) {
                AProxyI.onRequestSuccess(requestId, code, result)
            } else {
                AProxyI.onRequestFailed(requestId, code, result)
            }
            result
        } catch (e: Exception) {
            AProxyI.onRequestFailed(requestId, 0, e)
    ...
    class RequestTracker(private val requestId: String) {
        ...
        fun success(
            ...
            AProxyI.onRequestSuccess(requestId, responseCode, response, headers)
        }
        ...
        fun failed(
            ...
            AProxyI.onRequestFailed(requestId, responseCode, error)
        }
        ...
        fun failed(exception: Throwable) {
            AProxyI.onRequestFailed(requestId, 0, exception)
        }
         ...
         */
        fun cancelled() {
            AProxyI.onRequestCancelled(requestId)
        }
    }
}
